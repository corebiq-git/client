(() => {
  ...
  function resolve(spec, parent) {
    ...
    if (!/\.(tsx?|jsx?|css|mjs|js)$/.test(u.pathname)) {
      const fallbackExtensions = ['.ts', '.tsx', '.js', '.jsx', '.mjs'];
      const firstCandidate = u.href + fallbackExtensions[0];
  ...
  async function loadModule(url) {
    ...
    if (!/\.[a-z0-9]+$/i.test(url)) {
      ['.ts', '.tsx', '.js', '.jsx', '.mjs'].forEach((ext) => candidateUrls.add(url + ext));
    }
